...
def _sql_objects_estimate_aggregates(as_work, as_mat, as_profit):
    ...
    return (
        ...
        "FROM objects o "
        "LEFT JOIN ("
        "  SELECT object_id, user_id, estimate_id FROM ("
        "    SELECT object_id, user_id, MAX(id) AS estimate_id, 1 AS pri FROM estimates WHERE status = 'Утверждена' GROUP BY object_id, user_id"
        "    UNION ALL"
        "    SELECT object_id, user_id, MAX(id) AS estimate_id, 2 AS pri FROM estimates WHERE status = 'Отправлена' GROUP BY object_id, user_id"
        "    UNION ALL"
        "    SELECT object_id, user_id, MAX(id) AS estimate_id, 3 AS pri FROM estimates WHERE status = 'Черновик' GROUP BY object_id, user_id"
        "  ) u"
        "  JOIN ("
        "    SELECT object_id, user_id, MIN(pri) AS min_pri FROM ("
        "      SELECT object_id, user_id, 1 AS pri FROM estimates WHERE status = 'Утверждена'"
        "      UNION ALL"
        "      SELECT object_id, user_id, 2 AS pri FROM estimates WHERE status = 'Отправлена'"
        "      UNION ALL"
        "      SELECT object_id, user_id, 3 AS pri FROM estimates WHERE status = 'Черновик'"
        "    ) x GROUP BY object_id, user_id"
        "  ) m ON m.object_id = u.object_id AND m.user_id = u.user_id AND m.min_pri = u.pri"
        ") pick ON pick.object_id = o.id AND pick.user_id = o.user_id "
        "LEFT JOIN estimates e ON e.id = pick.estimate_id AND e.user_id = o.user_id "
