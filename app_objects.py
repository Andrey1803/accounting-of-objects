...
def _object_work_dates_for_crew_sync(obj, fallback_dates):
    """Даты выезда для назначения бригады: календарь объекта ∪ даты из диспетчера."""
    days = set()
...
def _integration_resolve_status(existing_status, incoming_status, *, apply_sync: bool) -> str:
    """Статус из диспетчера: только при sync_status; ручные статусы учёта не затираем."""
    ...
    if existing in OBJECT_STATUSES_OA_MANUAL:
        return existing
    if incoming in OBJECT_STATUSES_DISPATCHER_SYNC:
        return incoming
